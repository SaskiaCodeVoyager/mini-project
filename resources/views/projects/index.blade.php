    ...
    <div x-show="openCreateModal" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <form action="/projects" method="POST">
                @csrf
                <select id="multiNama" name="nama[]" class="w-full border p-2 select2" multiple>
        ...
        <div class="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <form :action="'/projects/' + projectId" method="POST">
                ...
                <select id="multiNama" name="nama[]" class="w-full border p-2 select2" multiple>
                    @foreach($users as $user)
                        <option value="{{ $user->id }}"
                            @if($projects->first() && $project->users->contains($user)) selected @endif>
                            {{ $user->username }}
                        </option>
                    @endforeach
                </select>
                ...
                <select name="tahap_id" class="w-full p-2 border rounded mb-2 text-blue-800" x-model="projectTahap" required>
                    <option value="" disabled>Pilih Tahap</option>
                    @foreach($tahaps as $tahap)
                        <option value="{{ $tahap->id }}" @if($projects->first() && $project->tahap_id == $tahap->id) selected @endif>{{ $tahap->nama }}</option>
                    @endforeach
                </select>

                <textarea name="deskripsi" placeholder="Deskripsi" class="w-full p-2 border rounded mb-2 text-blue-800" x-model="projectDesc" required>
                    {{ $project->deskripsi ?? '' }}
                </textarea>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        // Inisialisasi Select2 pada elemen dengan ID multiNama
        $('#multiNama').select2({
